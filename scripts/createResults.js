...
const getResultsContent = results =>
    libraries
        .map(
            key =>
                `|${key}|${results[key].reduce(
                    (a = '', result) => `${a}${typeof result === 'number' ? `${result.toFixed(2)} ms` : result}|`,
                    ''
                )}`
        )
        .reduce((a = '', b) => `${a}${b}\n`, '')
...
const parseBuildResults = async () => {
    ...
    await libraries.forEach(library => {
        ...
        buildStats[library] = [`${data.time} ms`, `${data.assets.reduce((a, b) => a + b.size, 0) / 1000} KB`]
    })
